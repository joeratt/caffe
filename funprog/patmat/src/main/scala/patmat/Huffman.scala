...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case List() => Nil
    case c :: cs => {
      countNum(chars, chars.distinct)
      /* val pair: (Char, Int) = (c, 1)
      val temp: List[(Char, Int)] = times(cs)
      
      temp match {
        case Nil => List(pair)
        case (c,n) :: cs2 => (c,n+1)::cs2 
        case default => {
          pair :: temp
        }
      } 
    }
    case default => throw new Error("no")*/
    }
  }

  def countNum(chars: List[Char], distinctChars: List[Char]): List[(Char, Int)] = distinctChars match {
    case List() => Nil
    case c :: cs => {
      val pair: (Char, Int) = (c, chars.count((ch: Char) => ((chars.contains(ch) && c == ch))))
      pair :: countNum(chars, cs)
    }
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case List() => Nil
    case lf :: lvs => {
      val leaf: Leaf = Leaf(lf._1, lf._2)
      insert(leaf, makeOrderedLeafList(freqs.tail))
    }
  }
  ...
  def insert(leaf: Leaf, leaves: List[Leaf]): List[Leaf] = leaves match {
    case List() => List(leaf)
    case y :: ys => {
      if (leaf.weight <= y.weight) leaf :: leaves
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    ...
    case c :: Nil => trees
    case t1 :: t2 :: theRest =>
      insertCodeTree(Fork(t1, t2, chars(t1) ::: chars(t2), weight(t1) + weight(t2)), theRest)
  }

  def insertCodeTree(combinedTree: Fork, remainingTree: List[CodeTree]): List[CodeTree] = remainingTree match {
    case List() => List(combinedTree)
    case y :: ys => {
      if (combinedTree.weight <= weight(y)) combinedTree :: remainingTree
      else y :: insertCodeTree(combinedTree, ys)
    }
  }
   ...
   */
  def until(isSingleton: List[CodeTree] => Boolean, combineThemTrees: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = trees match {
    case List() => List()
    case c :: cs => {
   ...
   */
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = traverseTree(tree, bits, tree)

  def traverseTree(mainTree: CodeTree, bitsProgression: List[Bit], currentTree: CodeTree): List[Char] = currentTree match {
